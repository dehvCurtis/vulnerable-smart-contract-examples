...
/**
 * @title Advanced Delegatecall Patterns Test
...
// ============================================================================
// 5. LIBRARY DELEGATECALL PATTERNS
// ============================================================================

library StatefulLibrary {
    // VULNERABILITY: Library with state variables
    // This creates storage collision risk when used with delegatecall
    struct LibraryStorage {
        uint256 counter;
        address owner;
    }

    function increment(LibraryStorage storage self) internal {
        self.counter++;
    }

    function dangerousDelegate(address target, bytes memory data) internal {
        (bool success, ) = target.delegatecall(data);
        require(success, "Library delegate failed");
    }
}

contract LibraryDelegatecallUser {
    using StatefulLibrary for StatefulLibrary.LibraryStorage;

    StatefulLibrary.LibraryStorage public libraryState;
    address public implementation;

    // VULNERABLE: Using library function that does delegatecall
    function libraryExecute(address target, bytes memory data) public {
        // VULNERABILITY: Library delegatecall without validation
        StatefulLibrary.dangerousDelegate(target, data);
    }

    // VULNERABLE: External library call via delegatecall
    function externalLibraryDelegate(address library, bytes memory data) public {
        // VULNERABILITY: Delegatecall to external library
        (bool success, ) = library.delegatecall(data);
        require(success, "External library failed");
    }
}

// ============================================================================
// 6. COMPLEX DELEGATECALL PATTERNS
// ============================================================================
